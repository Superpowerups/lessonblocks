...
class CoursePlayerManager {
  constructor() {
    ...
    this.lessonTimer = null; // Will hold timer interval
    this.timerElement = null; // Will hold timer UI element
    this.timerConfig = null; // Will hold timer configuration
    this.timerState = 'inactive'; // 'inactive', 'running', 'warning', 'expired'
    this.init();
  }
  ...
  setupCoursePlayerHooks() {
    ...
    CoursePlayerV2.on('hooks:contentDidChange', (data) => {
      ...
      // Check for lesson timer (with delay to ensure page is fully rendered)
      setTimeout(() => {
        this.checkForLessonTimer();
      }, 500);
  ...
  checkForLessonTimer() {
    // Look for div with data-lesson-timer attribute on the current page
    const timerElement = document.querySelector('[data-lesson-timer]');
    
    if (timerElement) {
      console.log('Lesson timer element found on current page');
      this.initializeLessonTimer(timerElement);
    } else {
      console.log('No lesson timer element found on current page');
    }
  }

  initializeLessonTimer(timerElement) {
    // Extract timer configuration from attributes
    const time = parseInt(timerElement.getAttribute('data-time')) || 120;
    const timeWarn = parseInt(timerElement.getAttribute('data-time-warn')) || 30;
    const heading = timerElement.getAttribute('data-heading') || 'You have {{time_left}} to complete the lesson';
    const expiredHeading = timerElement.getAttribute('data-time-expired-message-heading') || 'Time expired';
    const expiredSubheading = timerElement.getAttribute('data-time-expired-message-subheading') || 'Reload this lesson to start over';
    const colorNormal = timerElement.getAttribute('data-color-normal') || '#10b981';
    const colorWarning = timerElement.getAttribute('data-color-warning') || '#f59e0b';
    const colorExpired = timerElement.getAttribute('data-color-expired') || '#ef4444';
    const minutesText = timerElement.getAttribute('data-minutes-text') || 'minutes';
    const secondsText = timerElement.getAttribute('data-seconds-text') || 'seconds';
    const warningAnimation = timerElement.getAttribute('data-warning-animation') || 'pulse'; // pulse, shake, flash, bounce, none

    this.timerConfig = {
      totalTime: time,
      warningTime: timeWarn,
      heading,
      expiredHeading,
      expiredSubheading,
      colorNormal,
    ...
    console.log('Timer configuration:', this.timerConfig);

    // Hide complete button immediately
    this.hideCompleteButton();
  ...
  createTimerUI() {
    // Find the title element to add timer to
    const titleElement = document.querySelector('.course-player__content-header__title');
    if (!titleElement) {
      console.warn('Could not find course player title element for timer UI');
      return;
    }

    // Add timer animations CSS if not already added
    if (!document.getElementById('lesson-timer-animations')) {
      this.addTimerAnimations();
    }

    // Create timer span element
    this.timerElement = document.createElement('span');
    this.timerElement.className = 'lesson-timer-badge';
    this.timerElement.style.cssText = `
      background: ${this.timerConfig.colorNormal};
      color: white;
      padding: 4px 12px;
      border-radius: 12px;
      font-size: 12px;
    ...
    // Add to title
    titleElement.appendChild(this.timerElement);
  ...
  updateTimerDisplay() {
    if (!this.timerElement || !this.timerConfig) return;

    const { remainingTime, heading, minutesText, secondsText, colorNormal, colorWarning, colorExpired, warningTime, warningAnimation } = this.timerConfig;
    ...
    const displayText = heading.replace('{{time_left}}', timeDisplay);
    ...
    if (remainingTime <= 0) {
      this.timerElement.style.backgroundColor = colorExpired;
      this.timerElement.style.animation = 'none';
      this.timerState = 'expired';
    } else if (remainingTime <= warningTime) {
      this.timerElement.style.backgroundColor = colorWarning;
      this.timerState = 'warning';
      
      // Apply warning animation if just entered warning state
      if (previousState !== 'warning' && warningAnimation !== 'none') {
        this.timerElement.style.animation = `timer-${warningAnimation} 1s ease-in-out infinite`;
      }
    } else {
      this.timerElement.style.backgroundColor = colorNormal;
      this.timerElement.style.animation = 'none';
  ...
  startLessonTimer() {
    ...
    this.timerState = 'running';
    console.log('Starting lesson timer');

    this.lessonTimer = setInterval(() => {
      this.timerConfig.remainingTime--;
      this.updateTimerDisplay();

      if (this.timerConfig.remainingTime <= 0) {
        this.expireLessonTimer();
      }
    }, 1000);
  }
  ...
  expireLessonTimer() {
    ...
    this.timerState = 'expired';
    console.log('Lesson timer expired');

    // Create expired overlay
    this.createExpiredOverlay();
  }
  ...
  createExpiredOverlay() {
    ...
    if (!mainContent) {
      console.warn('Could not find main content element for timer overlay');
      return;
    }

    // Ensure main content has relative positioning for absolute overlay
    const originalPosition = mainContent.style.position;
    if (!originalPosition || originalPosition === 'static') {
      mainContent.style.position = 'relative';
    }

    // Create a wrapper to contain all children for blur effect
    const contentWrapper = document.createElement('div');
    contentWrapper.id = 'lesson-timer-content-wrapper';
    ...
    const overlay = document.createElement('div');
    overlay.id = 'lesson-timer-expired-overlay';
    overlay.style.cssText = `
      position: absolute;
      top: 0;
      left: 0;
    ...
    // Create message container (not blurred)
    const messageContainer = document.createElement('div');
    messageContainer.style.cssText = `
      background: white;
      padding: 40px;
    ...
    messageContainer.innerHTML = `
      <h2 style="margin: 0 0 16px 0; color: #ef4444; font-size: 24px; font-weight: 600;">
        ${this.timerConfig.expiredHeading}
      </h2>
      <p style="margin: 0; color: #6b7280; font-size: 16px; line-height: 1.5;">
        ${this.timerConfig.expiredSubheading}
      </p>
    `;
    ...
    console.log('Timer expired overlay created with blur effect');
  }
  ...
  clearLessonTimer() {
    ...
    if (this.lessonTimer) {
      clearInterval(this.lessonTimer);
      this.lessonTimer = null;
      console.log('Lesson timer cleared');
    }
    ...
    if (this.timerElement) {
      ...
      console.log('Timer UI removed');
    }

    // Remove expired overlay and unwrap content
    const expiredOverlay = document.getElementById('lesson-timer-expired-overlay');
    const contentWrapper = document.getElementById('lesson-timer-content-wrapper');
    const mainContent = document.querySelector('main') || document.getElementById('main-content');
    
    if (expiredOverlay) {
      expiredOverlay.remove();
      console.log('Expired overlay removed');
    }
    
    // Unwrap the blurred content if wrapper exists
    if (contentWrapper && mainContent) {
      // Move all children from wrapper back to main content
      while (contentWrapper.firstChild) {
        mainContent.appendChild(contentWrapper.firstChild);
      }
      // Remove the wrapper
      contentWrapper.remove();
      console.log('Content unwrapped and blur removed');
    }
  ...
  addTimerAnimations() {
    ...
    style.id = 'lesson-timer-animations';
    style.textContent = `
